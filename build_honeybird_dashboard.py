...
@media(max-width:768px){
  ...
  .header h1 span.bee{font-size:22px}
  .entity-info{font-size:10px}
  .kpi-grid{grid-template-columns:repeat(2,1fr);gap:8px}
  .kpi{padding:14px}
  .kpi .label{font-size:11px}
  .kpi .value{font-size:18px}
  .grid-2,.grid-3{grid-template-columns:1fr}
  .tab-bar{padding:0 8px;gap:2px;scroll-snap-type:x mandatory}
  .tab-btn{font-size:12px;padding:10px 14px;min-height:44px;scroll-snap-align:start}
  .filter-row{flex-direction:column;gap:8px}
  .filter-row select,.filter-row input{width:100%;min-height:44px;font-size:14px}
  ...
  .sub-btn{min-height:40px;padding:8px 16px;font-size:13px}
  .modal{max-width:95vw;padding:16px}
  .btn{min-height:44px;padding:10px 20px;font-size:14px}
  .theme-toggle{min-height:40px;padding:8px 16px}
  .pagination button{min-height:40px;min-width:40px;font-size:14px}
  select{-webkit-appearance:none;appearance:none}
  .tbl-scroll{margin:0 -14px;padding:0 14px}
}
/* === MOBILE (480px) === */
@media(max-width:480px){
  ...
  .header h1 span.bee{font-size:20px}
  .entity-info{font-size:9px}
  .quote-bar{font-size:11px;padding:4px 10px}
  .kpi-grid{grid-template-columns:1fr;gap:6px}
  .kpi{padding:12px;display:flex;justify-content:space-between;align-items:center}
  .kpi .label{font-size:12px;margin-bottom:0}
  .kpi .value{font-size:18px}
  .tab-bar{gap:1px;margin-bottom:12px;padding:0 4px}
  .tab-btn{font-size:11px;padding:10px 10px;min-height:44px;white-space:nowrap}
  .card{padding:10px;border-radius:8px}
  .chart-box{padding:10px;border-radius:8px}
  .chart-box canvas{max-height:200px!important}
  .chart-box h3{font-size:12px;margin-bottom:8px}
  table{font-size:10px}
  th{padding:8px 6px;font-size:10px}
  td{padding:6px}
  .recon-table{font-size:9px}
  .recon-table th,.recon-table td{padding:6px 4px}
  .info-grid{gap:4px;font-size:12px}
  .modal{padding:12px;border-radius:8px}
  .modal h3{font-size:16px}
  .coming-soon h2{font-size:36px}
  .coming-soon p{font-size:14px}
  .pagination{gap:4px}
  .pagination button{min-height:36px;min-width:36px;font-size:12px;padding:4px 8px}
  .filter-row select,.filter-row input{font-size:16px;min-height:48px}
  .sub-btn{min-height:44px;font-size:12px;padding:8px 12px}
...
@media(max-width:360px){
  ...
  .tab-btn{font-size:10px;padding:8px 7px}
...
@media(hover:none) and (pointer:coarse){
  .tab-btn,.sub-btn,.btn,.theme-toggle,.pagination button{min-height:44px}
  select,input[type="text"],input[type="date"]{min-height:48px;font-size:16px}
  .kpi{min-height:60px}
}
"""
...
// ========== BS vs IS HELPER ==========
// CRITICAL: Only Income Statement items count for P&L / revenue / expenses.
// Balance Sheet items (loans, shareholder contributions, interaccount transfers) are NOT income/expenses.
function isIS(txn){return (txn.allocation||'')==='Income Statement';}
function isBS(txn){return (txn.allocation||'')==='Balance Sheet';}
...
function renderOverview(){
  ...
  // Monthly income/expenses (current month) — IS items ONLY
  var mInc=0,mExp=0;
  for(var i=0;i<TXN.length;i++){
    if(!isIS(TXN[i]))continue;
  ...
  // 12-month trend chart — IS items ONLY
  ...
  // Expense categories donut — IS items ONLY
  var catTotals={};
  for(var i=0;i<TXN.length;i++){
    if(!isIS(TXN[i]))continue;
    if((TXN[i].amount||0)<0){
...
function renderIncomeStatement(){
  var fyMonths=getFYMonths();
  // Collect revenue and expense categories by month — IS ITEMS ONLY
  // BS items (loans, shareholder contributions, interaccount transfers) are excluded
  var revCats={},expCats={};
  for(var i=0;i<TXN.length;i++){
    var t=TXN[i];
    if(!isIS(t))continue;
...
function renderSpending(){
  // Expense breakdown by category — IS items ONLY (no BS transfers)
  var catTotals={};
  var fyMonths=getFYMonths();
  var monthlyCats={};
